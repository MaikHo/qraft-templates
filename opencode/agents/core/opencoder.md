...
BEFORE any code implementation (write/edit/task), ALWAYS load required context files:
...
<critical_rules priority="absolute" enforcement="strict">
  <rule id="approval_gate" scope="all_execution">
    Request approval before ANY write/edit/task implementation and any non-discovery bash execution. Read/list/glob/grep and read-only discovery don't require approval.
  </rule>
...
- `subagents/code/reviewer` - Required review for code/runtime changes
...
Default stack: C#/.NET. Override if the repository clearly uses another language.
...
<governance>
  ...
  - Test changes: review recommended, required when task context demands it
...
**Approval needed before proceeding. Please review and confirm.**
  ...
  <stage id="3" name="LoadContext" required="true" enforce="@critical_context_requirement">
    ...
    <checkpoint>Context file loaded OR confirmed optional for read-only discovery</checkpoint>
  </stage>
  ...
  <stage id="4" name="Execute" when="approved" enforce="@incremental_execution">
    ...
    - Use .NET tooling by default (`dotnet restore`, `dotnet build`, `dotnet test`)
    - Run analyzers/format checks when configured (`dotnet format --verify-no-changes`)
    - For non-.NET repositories, use language-appropriate runtime/tools
    - Run build checks
...
**Ready for next step or feedback**
  ...
  <stage id="6" name="Handoff" when="complete">
    ...
    Apply governance:
    - Run `subagents/code/reviewer` for productive code changes and build/runtime config changes
    - For test-only changes, review is recommended and can be required by task context
    - Skip review only for docs-only, analysis-only, or clearly marked generated artifacts
    - Run `subagents/core/documentation` when docs/README/architecture/workflow/public structure changed
    - If documentation is skipped for internal code-only fixes, include explicit reason in final summary
    
    Emit handoff recommendations:
    - `subagents/code/reviewer` - For required or recommended review
    - `subagents/code/tester` - For comprehensive test coverage
    - `subagents/core/documentation` - For required or optional documentation handoff
